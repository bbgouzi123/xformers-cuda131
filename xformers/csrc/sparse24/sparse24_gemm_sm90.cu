...
torch::stable::Tensor _sparse24_fp8_sm90_cutlass_gemm(
  ...
  if (!kIsMeta) {
    device_guard.emplace(torch::stable::device(tensor_a).index());
  }
  ...
  STD_TORCH_CHECK(torch::stable::device(tensor_a) == torch::stable::device(tensor_b));
  STD_TORCH_CHECK(torch::stable::device(tensor_a) == torch::stable::device(tensor_e));
  ...
  if (a_scale.has_value()) {
    ...
    STD_TORCH_CHECK(torch::stable::device(*a_scale) == torch::stable::device(tensor_a));
  ...
  if (b_scale.has_value()) {
    ...
    STD_TORCH_CHECK(torch::stable::device(*b_scale) == torch::stable::device(tensor_b));
   ...
   */
  args.hw_info.device_id = torch::stable::device(tensor_a).index();
...
_sparse24_sm90_cutlass_compress_t(torch::stable::Tensor a) {
  std::optional<torch::stable::accelerator::DeviceGuard> device_guard;
  if (!kIsMeta) {
    device_guard.emplace(torch::stable::device(a).index());
  }
  ...
  hw_info.device_id = torch::stable::device(a).index();
