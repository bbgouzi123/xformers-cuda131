...
def get_cuda_version(cuda_dir) -> int:
    # 首先尝试从 PyTorch 获取 CUDA 版本（如果 nvcc 不可用）
    if cuda_dir is None:
        try:
            import torch
            if torch.cuda.is_available() and torch.version.cuda:
                cuda_version_str = torch.version.cuda
                # 解析版本字符串，例如 "13.0" -> 1300
                version_parts = cuda_version_str.split(".")
    ...
    try:
        ...
        # 如果 nvcc 不可用，使用 PyTorch 的 CUDA 版本
        import torch
        if torch.cuda.is_available() and torch.version.cuda:
            cuda_version_str = torch.version.cuda
            version_parts = cuda_version_str.split(".")
            if len(version_parts) >= 2:
                ...
                minor = int(version_parts[1])
                return major * 100 + minor
        raise RuntimeError("Cannot determine CUDA version. Please install CUDA Toolkit or ensure PyTorch can detect CUDA.")
...
def get_extensions():
    ...
    # Modified to support PyTorch 2.9.1+cu130 (CUDA 13.0 compatibility)
    if torch.__version__ < "2.9.1":
        raise RuntimeError(
            "This version of xFormers requires PyTorch 2.9.1+ (or PyTorch 2.10+). "
    ...
    # Add compatibility headers directory
    compat_dir = os.path.join(this_dir, "torch_compat")
    include_dirs = [extensions_dir, compat_dir]
    ext_modules = []
    cuda_version = None
    hip_version = None
    flash_version = "0.0.0"
